...
MPU9250::MPU9250(int8_t cspin, float mx_bias, float my_bias, float mz_bias, float mx_scale, float my_scale, float mz_scale) : MPU9250(cspin){
  magBias[0] = mx_bias;
  magBias[1] = my_bias;
  magBias[2] = mz_bias;
...
void MPU9250::magCalMPU9250(float * bias_dest, float * scale_dest)
{
  ...
  for (size_t x = 0; x < 3; x++) {
    if ((bias_dest[x] == 0) || (scale_dest[x] == 0)) {
      calib_needed = true;
      break;
    }
  }
  ...
  if (!calib_needed) {
    Serial.println(F("Mag calibration already exists, skipping."));
    return;
  }
